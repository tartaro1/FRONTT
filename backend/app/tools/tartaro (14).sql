...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 18-06-2024 a las 06:08:37
...
--
CREATE DATABASE IF NOT EXISTS `tartaro` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `tartaro`;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AddProducto` (IN `nombre` VARCHAR(200), IN `id_categoria` INT(11), IN `marca` VARCHAR(100), IN `id_proveedor` INT(11), IN `descripcion` VARCHAR(200), IN `precio` DECIMAL(10,3), IN `calificacion` INT(3), IN `imagen` TEXT)   BEGIN
INSERT INTO `productos`(`NombreProducto`, `ID_Categoria`, `Marca`, `ID_Proveedor`, `Descripcion`, `PrecioVenta`, `Calificacion`,  `imagen`) VALUES (nombre, id_categoria, marca, id_proveedor, descripcion, precio, calificacion, imagen);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_BuscarCategoria` (IN `_categoria` VARCHAR(200))   BEGIN
SELECT
...
DROP PROCEDURE IF EXISTS `SP_CREARCOPIA`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CREARCOPIA` (IN `NombreBD` VARCHAR(200), IN `VersionBD` VARCHAR(200), IN `Tipo` VARCHAR(200), IN `Ubicacion` VARCHAR(200), IN `Informacion` VARCHAR(200))   BEGIN
...
(7, '2024-06-13 12:39:18', 'Base de datos', 'V.1.5', 'Manual', '/backup/', 'backup mensual'),
(8, '2024-06-13 12:39:41', 'Base de datos', 'V.1.5', 'Manual', '/backup/', 'backup mensual');
...
INSERT INTO `pedidos` (`ID_Pedido`, `EstadoPedido`, `Direccion`, `Cliente`, `PrecioVenta`, `ID_Repartidor`, `FechaPedido`) VALUES
(2, 'En Espera', 'ffff', 42, 4.00, 49, '2024-06-08 15:11:52'),
(3, 'Entregado', 'gvbbbgggg', 49, 30.00, 41, '2024-06-15 22:00:57');
...
(1, 'Carulla', 'Calle 123', 123456789, 'proveedora@example.com'),
(2, 'Éxito', 'Avenida 456', 987654321, 'proveedorb@example.com'),
...
ALTER TABLE `pedidos`
  ...
  ADD KEY `repartidor` (`ID_Repartidor`);
...
ALTER TABLE `detallefactura`
  ...
  ADD CONSTRAINT `detallefactura_ibfk_2` FOREIGN KEY (`ID_Producto`) REFERENCES `productos` (`id`);
...
ALTER TABLE `pedidos`
  ...
  ADD CONSTRAINT `repartidor` FOREIGN KEY (`ID_Repartidor`) REFERENCES `usuarios` (`ID_Usuario`);
...
ALTER TABLE `productos`
  ...
  ADD CONSTRAINT `nombre_proveedor` FOREIGN KEY (`ID_Proveedor`) REFERENCES `proveedores` (`ID_Proveedor`);

--
-- Filtros para la tabla `usuarios`
--
ALTER TABLE `usuarios`
  ADD CONSTRAINT `usuarios_ibfk_1` FOREIGN KEY (`ID_Rol`) REFERENCES `cargo` (`ID_Rol`);
COMMIT;
